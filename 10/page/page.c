...
#define TAG "Lab10 - Page Allocation"
...
static int thread_function(void *args) {
    ...
    printk("Number of my struct elements, which will fit one kernel memory page: %ld", PAGE_SIZE/sizeof(m));
    printk("Number of pages (based on module parameter): 2^%d = %d", pow,number_of_pages);
    ...
    print_time(&end_time);
    printk("Time spent on memory allocation and deallocation: %lu seconds %lu nanoseconds", end_time.tv_sec - current_time.tv_sec,
